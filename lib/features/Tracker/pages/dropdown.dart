...
class DropDownPage extends StatefulWidget {
  ...
  const DropDownPage({Key navKey}) : super(key: navKey);
  @override
...
class _DropDownPageState extends State<DropDownPage> {
  ...
  addItemDialog() async {
    return await showDialog(
      context: DropDownPage.navKey.currentState?.overlay?.context ?? context,
  ...
  Widget build(BuildContext context) {
              ...
              addItemDialog().then((value) async {
                if (value != null &&
                    editableItems.indexWhere(
                        ...
                        -1) {
                  Navigator.pop(
                      DropDownPage.navKey.currentState?.overlay?.context ??
                          context);
                  updateParent(value);
                }
              });
      ...
      "Single dialog dark mode": Card(
        ...
        child: SearchChoices.single(
          ...
          closeButton: TextButton(
            onPressed: () {
              Navigator.pop(
                  DropDownPage.navKey.currentState?.overlay?.context ?? context);
            },
            child: Text(
              "Close",
              style: TextStyle(color: Colors.white),
    ...
    return MaterialApp(
      ...
      navigatorKey: DropDownPage.navKey,
      home: asTabs
